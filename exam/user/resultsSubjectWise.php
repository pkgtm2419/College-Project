...
<li class="listyle liactive"><a href="#">Subject Wise</a></li>
<li class="listyle"><a href="resultsDifficultyWise.php?event=<?php echo $_GET["event"];?>&tid=<?php echo $_GET['tid'];?>">Difficulty Wise</a></li>
</ul></div>
<div style="width:100%;">
<div style="font-size:13px;font-weight: bold;padding-left:20px;"><?php $get_test=mysql_fetch_array(mysql_query("select test_name,category_name from test_master t inner join category_master c on t.category_id=c.category_id where test_id=".$_GET["tid"]));echo $get_test[1]." > ".$get_test[0];?>&nbsp;>&nbsp;Subject Wise Analysis</div>
...
<div id="sugestion" class="sugestion"></div>
<br/><br/>
<div class="negativeMarks" style="font-size: 16px;font-weight: bold;padding-left:20px;">Negative marks scored in subjects</div>
<div id="loadingDivNegative" class="loadingDiv loadingDivHide"><img style="vertical-align:top;" src="images/busy.gif"/> Loading Chart. Please Wait...</div>
...
$.ajax({type:"GET",data:'event=<?php echo $_GET["event"];?>&AjaxRequestId='+Math.random()+'&tid=<?php echo $_GET['tid'];?>',url:"ajax/loadBarChart.php",success:function(result){
$("#getMainScripts").html(result);
$("#loadingDivMain").addClass("loadingDivHide");
}});
$("#loadingDivNegative").removeClass("loadingDivHide");
$.ajax({type:"GET",data:'event=<?php echo $_GET["event"];?>&AjaxRequestId='+Math.random()+'&tid=<?php echo $_GET['tid'];?>',url:"ajax/loadNegativeMarksBarChart.php",success:function(result){
$("#getNegativeMarksScript").html(result);
$("#loadingDivNegative").addClass("loadingDivHide");
}});
$.ajax({type:"GET",data:'event=<?php echo $_GET["event"];?>&AjaxRequestId='+Math.random()+'&tid=<?php echo $_GET['tid'];?>',url:"ajax/loadSugestions.php",success:function(result){
$("#sugestion").html(result);
}});
});</script><script src="js/amcharts.js" ></script><script type="text/javascript" src="js/serial.js" ></script><?php require_once('footer.php');?>